...
# checkpoints_dir = r'E:\python program'
# log_folder = r'E:\python program\log_folder'

checkpoints_dir = '/home/gpudouble1/zhangchao/fcn'
log_folder = '/home/gpudouble1/zhangchao/fcn/log_folder'
...
upsample_factor = 32
number_of_classes = 2
tfrecord_filename = 'train_data_2w.tfrecords'
...
combined_labels = tf.concat(values=[class_labels_tensor, background_labels_tensor], axis=2)
...
with tf.Session() as sess:
    ...
    # 25004 image, 15 epochs
    for i in range(25004*15):
        ...
        '''
        if i % 25004 == 0:
            # save_path = saver.save(sess, r"E:\python program\checkpoint\fcn32s_5e.ckpt")
            save_path = saver.save(sess, "/home/gpudouble1/zhangchao/fcn/checkpoint/fcn32s_15e.ckpt")
...
loss_all = open('loss_file_32s.txt', 'w')
loss_all.write(str(cross_entropy_plt))
loss_all.close()

plt.figure(figsize=(50, 6), dpi=300)
plt.subplot(1, 1, 1)
X = np.linspace(start=0, stop=6209*10, num=6209*10, endpoint=False)
Y = cross_entropy_plt

plt.plot(X, Y, color="blue", linewidth=1, linestyle='-', label="32s")
plt.ylim(0, 1)
plt.legend(loc='upper right')
plt.savefig('32s_loss.jpg')
#plt.show()
