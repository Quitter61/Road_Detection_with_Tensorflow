...
checkpoints_dir = '/home/damon/PycharmProjects/python/second_dataset/8s/fcn16s'
log_folder = '/home/damon/PycharmProjects/python/second_dataset/8s/log_folder'

slim = tf.contrib.slim
fcn_16s_checkpoint_path = os.path.join(checkpoints_dir, 'fcn16s_model5e_2.ckpt')
...
combined_labels = tf.concat(concat_dim=2,
                            values=[class_labels_tensor, background_labels_tensor])
...
with tf.variable_scope("fcn_8s")  as fcn_8s_scope:
    ...
    last_layer_upsample_logits_shape = tf.pack([last_layer_logits_shape[0],
    ...
    fused_last_layer_and_pool4_upsampled_by_factor_2_logits_shape = tf.pack([fused_last_layer_and_pool4_logits_shape[0],
    ...
    fused_last_layer_pool4_pool3_upsampled_by_8_logits_shape = tf.pack([fused_last_layer_and_pool4_and_pool3_logits_shape[0],
...
######此處出錯#######
#我reload 進來的是 fcn32s 不需要刪除最後的分類層權重
...
with tf.Session() as sess:
    ...
    for i in range(529 * 5):
        ...
        '''
        if i % 529 == 0:
            save_path = saver.save(sess, "/home/damon/PycharmProjects/python/second_dataset/8s/checkpoint/fcn8s_model5e_2.ckpt")
